...
const reducer = (state = initialState, action) => {
  switch (action.type) {
    case types.FETCH_FORM_INSTANCE_MANUAL_SCHEDULINGS_SUCCESS: {
      ...
      const updState = rawArr.reduce((prev, curr) => ({
        ...prev,
        [curr.formInstanceId]: {
          ...(prev[curr.formInstanceId] || {}),
          [curr.sectionId]: {
            ...(prev[curr.formInstanceId] && prev[curr.formInstanceId][curr.sectionId] ? prev[curr.formInstanceId][curr.sectionId] : {}),
    ...
    case types.TOGGLE_ROW_SCHEDULING_STATUS_SUCCESS: {
      ...
      return {
        ...
        [formInstanceId]: {
          ...
          [sectionId]: {
            ...(state[formInstanceId] && state[formInstanceId][sectionId] ? state[formInstanceId][sectionId] : {}),
            [rowKey]: status,
          },
        },
      };
