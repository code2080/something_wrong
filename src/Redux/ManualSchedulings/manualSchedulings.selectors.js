...
import {
  manualSchedulingStatuses,
  manualSchedulingFormStatuses,
} from '../../Constants/manualSchedulingConstants';
import {
  SECTION_CONNECTED,
  SECTION_TABLE,
...
export const selectManualSchedulingStatusForRow = createSelector(
  mSState,
  (mS) => (formInstanceId, sectionId, rowKey) =>
    _.get(
      mS,
      `${formInstanceId}.${sectionId}.${rowKey}`,
      manualSchedulingStatuses.NOT_COMPLETED,
    ),
);
...
const calculateMSRows = (values, sections) =>
  (Object.keys(values) || [])
    ...
    .reduce(
      (rows, sectionId) => rows + (Object.keys(values[sectionId]) || []).length,
      0,
    );
...
export const selectManualSchedulingStatus = createSelector(
  ...
  (mS, formInstances, forms) => (formInstanceId, formId) => {
    try {
      ...
      const completedSchedulings = (Object.keys(formInstanceMS) || []).reduce(
        (c, sectionId) =>
          c +
          (Object.keys(formInstanceMS[sectionId]) || []).filter(
            (rowKey) =>
              formInstanceMS[sectionId][rowKey] ===
              manualSchedulingStatuses.COMPLETED,
          ).length,
        0,
      );
      ...
      return {
        completed: 0,
        totalRows: 0,
        status: manualSchedulingFormStatuses.NOT_STARTED,
      };
    }
  },
);
