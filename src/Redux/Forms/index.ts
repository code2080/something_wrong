...
export const formSelector = (id: string | null | undefined) => (state: IState): TForm | undefined => id ? state.forms.map[id] || undefined : undefined;
export const formsLoading = (state: IState): boolean => state.forms.loading;

export const selectSectionInForm = (formId: string, sectionId: string) => (state: IState) => {
  return state.forms.map[formId]?.sections.find((section: any) => section._id === sectionId) || undefined;
}

export const selectTimeslotsForSectionInForm = (formId: string, sectionId: string) => (state: IState) => {
  const section = state.forms.map[formId]?.sections.find((section: any) => section._id === sectionId) || undefined;
  return section?.calendarSettings?.timeslots || [];
};

export const selectFormHasWeekPatternEnabled = (formId: string) => (state: IState) => {
  /**
  ...
  return !!form.sections.find((section) => {
    return !!(section.activityTemplatesSettings?.duration && section.activityTemplatesSettings?.datasource && section.activityTemplatesSettings?.weekPicker);
  });
}
...
export const fetchForms = () =>
  async (dispatch: any) => {
    try {
      ...
      const result = await api.get({ endpoint: `forms`, service: EExternalServices.PREFERENCES_BE });
