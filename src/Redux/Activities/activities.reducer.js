  ...
  switch (action.type) {
    case types.REORDER_ACTIVITIES_REQUEST: {
      ...
      const updActivities = activities.map(activity => {
        ...
        if (direction > 0 && activity.sequenceIdx > sourceIdx && activity.sequenceIdx <= destinationIdx) { return { ...activity, sequenceIdx: activity.sequenceIdx - 1 }; }
    ...
    case types.FETCH_ACTIVITIES_FOR_FORM_INSTANCE_SUCCESS: {
      ...
      const activities = activitityObjs
        .map((el, idx) => new Activity({ ...el, sequenceIdx: el.sequenceIdx == null ? idx : el.sequenceIdx }));
    ...
    case types.SAVE_ACTIVITIES_FOR_FORM_INSTANCE_SUCCESS: {
      const activitityObjs = action.payload.activities || [];
      const activities = activitityObjs
        .map((el, idx) => new Activity({ ...el, squenceIndex: el.sequenceIdx == null ? idx : el.sequenceIdx }));
      const { payload: { actionMeta: { formId, formInstanceId } } } = action;
      return {
        ...state,
        [formId]: {
