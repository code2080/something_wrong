...
export const extractSubmissionData = (submissions, cols) => {
   ...
   */
  return submissions.reduce((extractedData, submission) => {
    ...
    return {
      ...
      [submission._id]: (cols || []).reduce((row, col) => {
        ...
        const elementIdx = values[sectionId]
          ? values[sectionId].findIndex((el) => el.elementId === elementId)
          : -1;
      ...
      .map((section) => ({
        ...
        children: section.elements
          .filter(
            (el) =>
              el.datasource &&
              el.datasource.split(',')[1] &&
              el.datasource.split(',')[1] === 'object',
          )
...
export const getPayloadForExtraObject = ({
  ...
  try {
    ...
    if (extraObject[0] === 'scopedObject') {
      ...
      const value = findElementValueInSubmissionFromId(
        extraObject[1],
        extraObject[0],
        form.sections,
        formInstance.values,
      ...
      return [
        {
          ...
          extId: Array.isArray(value) ? value[0] : value,
        },
 ...
 */
export const getSelectionSettingsTECorePayload = (
  selectionSettings,
  form,
  formInstance,
  event,
) => {
  const extraObjectsPayload = (
    [['scopedObject'], ...selectionSettings.extraObjects] || []
  ).map((extraObject) =>
    getPayloadForExtraObject({ extraObject, form, formInstance }),
  );
    ...
    .map((includedField) => [
      {
        ...
        extId: includedField.fieldExtId,
      },
      {
        valueType: datasourceValueTypes.FIELD_VALUE,
        value: event[includedField.element],
      },
    ]);
 ...
 */
export const applyScopedObjectFilters = (
  formInstance,
  scopedObjects,
  filters,
) => {
  ...
  const scopedObject = scopedObjects.find(
    (obj) => obj.extid === scopedObjectExtId,
  );
  if (!scopedObject) return false;

  const filterFieldExtids = Object.keys(filters).filter(
    (fieldExtid) => filters[fieldExtid].length > 0,
  );
  return filterFieldExtids.some((fieldExtid) => {
    const field = scopedObject.fields.find(
      (field) => field.extid === fieldExtid,
    );
    if (!field) return false;
    const fieldQuery = filters[fieldExtid].toString().toLowerCase();
    return field.values.some((value) =>
      value.toLowerCase().includes(fieldQuery),
    );
  });
};
 ...
 * @returns Bool
 */
export const filterFormInstancesOnAuthedUser = (formInstance, userId) =>
  (formInstance.teCoreProps.assignedTo || []).includes(userId);
 ...
 * @returns Array
 */
export const parseTECoreGetObjectsReturn = (results) =>
  results.map((obj) => ({
    extid: obj.extid,
