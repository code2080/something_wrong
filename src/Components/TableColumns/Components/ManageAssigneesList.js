...
const User = ({ isSelf, user, onToggleUser, isAssigned }) => {
  return (
    <List.Item
      className={`assignment__popover--item ${
        isAssigned ? 'assigned-user' : 'user'
      } ${isSelf ? 'is-self' : ''}`}
      onClick={isAssigned ? null : onToggleUser}
    >
      <Avatar className='assignment__popover__item__avatar'>
        {user.initials}
      </Avatar>
      <div className='assignment__popover__item__name'>{user.name}</div>
      {isAssigned && (
        <div
          className='assignment__popover__item__remove'
...
const ManageAssigneesList = ({ users, selfUID, isAssigned, onToggleUser }) => {
  return (
    <List
