...
const mapArrayToProperties = (arr: any[], labelMap: object | null = null) => {
  return Object.keys(arr).reduce((props: any, key: string) => {
    ...
    if (
      arr[key] &&
      arr[key] != null &&
      (!Array.isArray(arr[key]) || arr[key].length)
    )
      return [
        ...props,
...
const ActivityFilterModal = ({ isVisible, onClose }: Props) => {
  ...
  const filterInclusion = useSelector(selectActivityFilterInclusionForForm)(
    formId,
  );
   ...
   * MEMOIZED VALUES
   */
  const properties: TProperty[] = useMemo(
    () => [...mapDesignToProperties(design), ...mapExtraInfoToProperties()],
    [design],
  );
  const selectedValues: TProperty[] = useMemo(
    () => generateSelectedValuesFromFilter(filterValues),
    [filterValues],
  );
  const availableValues = useMemo(
    () =>
      createPropertySelectorProperties(
        selectedProperty,
        filterOptions,
        filterValues,
      ),
    [selectedProperty, filterOptions, filterValues],
  );
  ...
  const onRemoveFilterValue = (propertyValue: string) => {
    ...
    const idx = (filterValues[extId] || []).findIndex(
      (el) => el === propertyValue,
    );
    dispatch(
      updateFilter({
        ...
        key: extId,
        value: [
          ...filterValues[extId].slice(0, idx),
          ...filterValues[extId].slice(idx + 1),
        ],
      }),
    );
  };
  ...
  const onSetActivityFilterInclusion = (
        ...
        <div className='activity-filtering--settings'>
          ...
          <div className='activity-filtering--content'>
            <div className='activity-filtering--settings-group'>
              ...
              <Radio.Group
                onChange={(e) => onSetActivityFilterMode(e.target.value)}
                value={filterMode}
              >
            ...
            <div className='activity-filtering--settings-group'>
              ...
              <Radio.Group
                onChange={(e) => onSetActivityFilterInclusion(e.target.value)}
                value={filterInclusion}
              >
                <Radio value={EActivityFilterInclusion.SINGLE}>
                  Single activity
                </Radio>
                <Radio value={EActivityFilterInclusion.SUBMISSION}>
                  All in submission
                </Radio>
              </Radio.Group>
            </div>
